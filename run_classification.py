...
def save_results(params_list, freeze_test_set=True):
    ...
    for param_index, params in enumerate(params_list):
        ...
        _, all_label_probs, test_labels = get_model_response(params, all_train_sentences, all_train_labels, train_sentences, train_labels, val_sentences, val_labels, test_sentences, test_labels)
        ...
        acc_conc, entropy_conc, prob_conc, ECE_conc = eval_accuracy(all_label_probs,
                                                                                            test_labels,
                                                                                            all_train_sentences,
                                                                                            all_train_labels,
        ...
        acc_linc, entropy_linc, prob_linc, ECE_linc = eval_accuracy(all_label_probs, test_labels, all_train_sentences,
                                                           all_train_labels,
                                                           val_sentences, val_labels, params, lr, epochs,
                                                           curr_seed, mode="diagonal_W",
                                                           p_cf=p_cf)
        accuracies = [acc_original, acc_conc, acc_linc]
        ECE = [ECE_original, ECE_conc, ECE_linc]
        ...
        with open('entropy/PT_LinC_entropy_{}_{}_{}shots_seed{}.npy'.format(params['model'], params['dataset'], params['num_shots'], params['seed']), 'wb') as f:
            np.save(f, np.array(entropy_linc.flatten()))

        fig, ax = plt.subplots(1, 1, figsize=(7, 5.6))
        bins = np.linspace(0.0, 4.8, 7) - 0.4
        ax.set_xticks(np.linspace(0.0, 4.8, 7))
        n, bins, patches = ax.hist([entropy_original.flatten(), entropy_linc.flatten()], bins, alpha=0.6,
                                   weights=[np.ones(len(entropy_original)) / len(entropy_original),
                                            np.ones(len(entropy_linc)) / len(entropy_linc)],
                                   align='mid')
        hatches = ['\\\\', '//']
        colors = ['darkblue', 'red']
        ...
        ax.legend(['Vanilla LLM', 'LinC'], fontsize=24, loc='upper left')
...
def eval_accuracy(all_label_probs, test_labels, all_train_sentences, all_train_labels, val_sentences, val_labels,
    ...
    if p_cf is None:
        ...
        _, all_val_label_probs, val_labels = get_model_response_val(params, all_train_sentences, all_train_labels, train_sentences, train_labels, val_sentences, val_labels, val_sentences, val_labels)
...
def get_p_content_free(params, all_train_sentences, all_train_labels, train_sentences, train_labels, val_sentences, val_labels, test_labels, content_free_inputs=('N/A')):
    ...
    _, all_p_y, _ = get_model_response(params, all_train_sentences, all_train_labels, train_sentences, train_labels, val_sentences, val_labels, content_free_inputs, test_labels, normalize=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datasets', dest='datasets', action='store', required=False, default='sst2',
                        help='name of dataset(s), e.g., agnews')
    parser.add_argument('--num_seeds', dest='num_seeds', action='store', required=False, default=1,
    ...
    parser.add_argument('--val_seed', dest='val_seed', action='store', required=False, default=42,
    ...
    parser.add_argument('--epochs', dest='epochs', action='store', required=True, help='total numbr of epochs T',
                        type=int)
    parser.add_argument('--val_size', dest='val_size', action='store', required=False, default=300,
